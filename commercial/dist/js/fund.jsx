...
(function () {
  ...
  function TxView({ fund, partners, rows, people, totalUnits, currentTotalThb, color, partnerName, staleDays, onAdd, onDelete }) {
    ...
    const submit = () => {
      ...
      setName(''); setAmt(''); setAdding(false);
    };
      ...
      <>
        <div className="card card-pad" style={{ marginBottom: 12 }}>
          ...
          <div className="fund-form">
            ...
            <label className="field"><label>ชื่อหุ้นส่วน</label>
              {adding ? (
                <div style={{ display: 'flex', gap: 6 }}>
                  <input className="input" autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="พิมพ์ชื่อคนใหม่" />
                  <button type="button" className="btn btn-ghost btn-sm" title="กลับไปเลือกจากรายชื่อ" onClick={() => { setAdding(false); setName(''); }}><Icon name="close" size={13} /></button>
                </div>
              ) : (
                <select className="select" value={name} onChange={e => { if (e.target.value === '__new__') { setAdding(true); setName(''); } else setName(e.target.value); }}>
                  <option value="" disabled>— เลือกหุ้นส่วน —</option>
                  {partners.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
                  <option value="__new__">➕ เพิ่มคนใหม่…</option>
                </select>
              )}
            </label>
